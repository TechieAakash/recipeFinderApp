...
def add_favorite(recipe_id):
    try:
        ...
        cursor.execute("SELECT id FROM user_favorites WHERE user_id = %s AND recipe_id = %s", 
                      (user_id, recipe_id))
        if cursor.fetchone():
            cursor.close()
            conn.close()
...
def remove_favorite(recipe_id):
    try:
        ...
        cursor.execute("DELETE FROM user_favorites WHERE user_id = %s AND recipe_id = %s", 
                      (user_id, recipe_id))
...
if __name__ == '__main__':
    ...
    print("🚀 Starting Recipe Finder API server...")
    print("📍 Server running on: http://127.0.0.1:5000")
    print("🔐 Authentication endpoints:")
    print("   POST /register          - Register new user")
    print("   POST /login             - User login")
    print("   GET  /profile           - Get user profile")
    print("   PUT  /profile           - Update profile")
    print("   GET  /favorites         - Get user favorites")
    print("   POST /favorites/<id>    - Add favorite")
    print("   DELETE /favorites/<id>  - Remove favorite")
    print("📚 Recipe endpoints:")
